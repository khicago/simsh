...
func specUniq() engine.CommandSpec {
	return engine.CommandSpec{
		...
		Examples:       ExamplesFor("uniq"),
		DetailedManual: LoadEmbeddedManual("uniq"),
		Run:            runUniq,
	}
}

func runUniq(runtime engine.CommandRuntime, args []string) (string, int) {
	showCount := false
	...
	raw, out, code := loadUniqSource(runtime, filePath)
	if code != 0 {
		return out, code
	}
...
func loadUniqSource(runtime engine.CommandRuntime, filePath string) (string, string, int) {
