...
func TestStaticMountOperations(t *testing.T) {
	...
	children, err := m.ListChildren(context.Background(), "/docs")
	if err != nil {
	...
	if isDir, err := m.IsDirPath(context.Background(), "/docs/scripts"); err != nil || !isDir {
		t.Fatalf("IsDirPath(/docs/scripts) = (%t, %v), want (true, nil)", isDir, err)
	}
	if isDir, err := m.IsDirPath(context.Background(), "/docs/README.md"); err != nil || isDir {
		t.Fatalf("IsDirPath(/docs/README.md) = (%t, %v), want (false, nil)", isDir, err)
	}

	if got, err := m.ReadRawContent(context.Background(), "/docs/README.md"); err != nil || got != "# docs\n" {
		t.Fatalf("ReadRawContent(/docs/README.md) = (%q, %v), want (%q, nil)", got, err, "# docs\n")
	}

	filesUnder, err := m.CollectFilesUnder(context.Background(), "/docs")
	if err != nil {
	...
	oneFile, err := m.ResolveSearchPaths(context.Background(), "/docs/scripts/build.sh", false)
	if err != nil {
	...
	if _, err := m.ResolveSearchPaths(context.Background(), "/docs", false); err == nil {
		t.Fatalf("ResolveSearchPaths(/docs, false) unexpectedly succeeded")
	}

	dirMeta, err := m.DescribePath(context.Background(), "/docs/scripts")
	if err != nil {
	...
	fileMeta, err := m.DescribePath(context.Background(), "/docs/scripts/build.sh")
	if err != nil {
...
func TestBaselineCorpusMountExposesEmbeddedCases(t *testing.T) {
	...
	paths, err := m.CollectFilesUnder(context.Background(), "/test/core-strict/cases")
	if err != nil {
	...
	meta, err := m.DescribePath(context.Background(), "/test/core-strict/cases/echo-basic.sh")
	if err != nil {
...
func TestSysBinMountOperations(t *testing.T) {
	...
	children, err := m.ListChildren(context.Background(), contract.VirtualSystemBinDir)
	if err != nil {
	...
	if isDir, err := m.IsDirPath(context.Background(), contract.VirtualSystemBinDir); err != nil || !isDir {
		t.Fatalf("IsDirPath(%q) = (%t, %v), want (true, nil)", contract.VirtualSystemBinDir, isDir, err)
	}
	if isDir, err := m.IsDirPath(context.Background(), "/sys/bin/echo"); err != nil || isDir {
		t.Fatalf("IsDirPath(%q) = (%t, %v), want (false, nil)", "/sys/bin/echo", isDir, err)
	}
	if _, err := m.IsDirPath(context.Background(), "/elsewhere"); !errors.Is(err, contract.ErrUnsupported) {
		t.Fatalf("IsDirPath(%q) error = %v, want ErrUnsupported", "/elsewhere", err)
	}

	if got, err := m.ReadRawContent(context.Background(), "/sys/bin/echo"); err != nil || got != "echo manual" {
		t.Fatalf("ReadRawContent(/sys/bin/echo) = (%q, %v), want (%q, nil)", got, err, "echo manual")
	}
	if got, err := m.ReadRawContent(context.Background(), "/sys/bin/cat"); err != nil || got != "builtin: cat" {
		t.Fatalf("ReadRawContent(/sys/bin/cat) = (%q, %v), want (%q, nil)", got, err, "builtin: cat")
	}

	search, err := m.ResolveSearchPaths(context.Background(), contract.VirtualSystemBinDir, true)
	if err != nil {
	...
	meta, err := m.DescribePath(context.Background(), "/sys/bin/echo")
	if err != nil {
...
func TestExternalBinMountOperations(t *testing.T) {
	...
	children, err := m.ListChildren(ctx, contract.VirtualExternalBinDir)
	if err != nil {
	...
	if isDir, err := m.IsDirPath(ctx, contract.VirtualExternalBinDir); err != nil || !isDir {
		t.Fatalf("IsDirPath(%q) = (%t, %v), want (true, nil)", contract.VirtualExternalBinDir, isDir, err)
	}
	if isDir, err := m.IsDirPath(ctx, "/bin/missing"); err != nil || isDir {
		t.Fatalf("IsDirPath(%q) = (%t, %v), want (false, nil)", "/bin/missing", isDir, err)
	}

	if got, err := m.ReadRawContent(ctx, "/bin/rg"); err != nil || got != "rg manual" {
		t.Fatalf("ReadRawContent(/bin/rg) = (%q, %v), want (%q, nil)", got, err, "rg manual")
	}
	if got, err := m.ReadRawContent(ctx, "/bin/sed"); err != nil || got != "stream editor" {
		t.Fatalf("ReadRawContent(/bin/sed) = (%q, %v), want (%q, nil)", got, err, "stream editor")
	}
	if _, err := m.ReadRawContent(ctx, "/bin/missing"); err == nil {
		t.Fatalf("ReadRawContent(/bin/missing) unexpectedly succeeded")
	}

	files, err := m.CollectFilesUnder(ctx, contract.VirtualExternalBinDir)
	if err != nil {
	...
	if _, err := m.ResolveSearchPaths(ctx, contract.VirtualExternalBinDir, false); err == nil {
		t.Fatalf("ResolveSearchPaths(%q, false) unexpectedly succeeded", contract.VirtualExternalBinDir)
	}

	meta, err := m.DescribePath(ctx, "/bin/sed")
	if err != nil {
