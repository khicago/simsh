...
- **目录**: 使用 `pkg/builtin/commands/<cmd>/manual.md` 作为单一来源
- **文件**: 每个命令一个 markdown 文件，如 `commands/ls/manual.md`, `commands/grep/manual.md`, `commands/cat/manual.md` 等
- **格式**: 使用 frontmatter 标记元数据（name, synopsis, category），正文包含详细说明、flag 解释、示例、注意事项
- **加载**: 通过 `//go:embed commands/*/manual.md` 在 `pkg/builtin/manuals.go` 中加载

#### 3.3 man 命令重构
- **文件**: `pkg/builtin/op_help_manual.go`
- **变更**:
  - `man cmd` → 输出 Manual 一行摘要 + Tips + Examples + Use-When/Avoid-When（简洁模式）
  - `man -v cmd` → 输出完整 embed markdown（详细模式，自动去除 frontmatter）
...
| Sprint 3: man 系统 | ✅ 完成 | manual 单一来源（`commands/*/manual.md`）、`man -v`/`man --list`、摘要 Use-When/Avoid-When |
| Sprint 4: 文本处理 + 行为修正 | ✅ 完成 | wc/sort/uniq/diff 命令、grep exit code 修正、cat 默认无行号 + `-n` flag |
| Sprint 5: 引擎安全加固 | ✅ 完成 | 语句执行上限、Pipeline 中间结果限制、Dispatch 递归深度限制均已落地 |
...
- `pkg/builtin/commands/*/manual.md` — 命令手册单一来源
...
1. **localfs 符号链接防护增强**: 继续收紧边界场景（嵌套 symlink、父目录不存在、跨平台路径差异）
2. **write-limited 集成回归补强**: 覆盖 runtime 装配链路，确保 `ExecutionPolicy.MaxWriteBytes` 在不同入口行为一致
3. **诊断与审计可观测性**: 为关键拒绝路径补充可读错误与审计事件样例
