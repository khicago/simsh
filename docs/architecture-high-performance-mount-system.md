...
- `ls` / `tree` now consume `ListEntries` + `MountEntry.Meta` when available, and only fall back to older path-by-path inspection for fallback-eligible mounts such as `local_fast` or `local_heavy`.
- `grep` / `rg` now try `SearchContent` before resolve-then-read loops, and only fall back for fallback-eligible mounts when the runtime returns ordinary unsupported behavior.
- `mv` / `rm` / `mkdir` now try one `ApplyMutations` batch before sequential fallback, but that sequential fallback is reserved for fallback-eligible mounts rather than `remote_high_latency` refusal paths.
