...
| Paired uplift proof harness (`simsh` vs thin baseline) | SHOULD | Need one downstream proof layer that keeps the deterministic probe agent, paired task manifest, and budgets fixed while comparing full `simsh` against one repo-controlled thin baseline substrate; use when validating retries, wasted steps/tokens, and environment-misunderstanding deltas without turning the repo into a generic benchmark zoo | `benchmarks/paired_uplift/*.go`, `benchmarks/paired_uplift/task_set.json`, `benchmarks/paired_uplift/reports/*`, `docs/architecture-paired-ab-uplift-proof-harness.md` |
| External outcome proof breadcrumbs | SHOULD | Need downstream proof reports to classify command-surface misunderstandings from `ExecutionTrace.ExternalOutcomes` before falling back to compatibility text, while preserving classification source, outcome kind, exit code, and safe virtual command-path breadcrumbs without serializing host-local provider paths | `benchmarks/paired_uplift/tasks.go`, `benchmarks/paired_uplift/models.go`, `benchmarks/paired_uplift/reports/*`, `docs/architecture-paired-ab-uplift-proof-harness.md` |
| Version-line release-readiness closeout | SHOULD | Need one explicit closeout checklist that ties release-facing docs, migration guidance, benchmark evidence, and cut criteria together without reopening feature scope during release preparation | `docs/notes-v0-3-0-release-readiness.md`, `docs/notes-v0-2-x-to-v0-3-0-migration.md`, `README.md` |
...
| Mount unsupported classification (`MountUnsupportedError` + `AllowsUnsupportedFallback`) | SHOULD | Need one shared way to distinguish ordinary unsupported fallback from explicit `remote_high_latency` refusal so builtin and engine helpers do not silently degrade high-latency mounts into fanout loops | `pkg/contract/mount_unsupported.go`, `pkg/contract/mount_dispatch.go`, `pkg/builtin/op_listing.go`, `pkg/builtin/op_json.go`, `pkg/builtin/search_scan.go` |
