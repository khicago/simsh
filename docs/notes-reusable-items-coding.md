...
| Adapter mount conformance harness | SHOULD | Need reusable `VirtualMount` list/search/describe/read-only metadata assertions across multiple adapter shapes without turning benchmark or adapter-local tests into duplicated mount smoke checks | `pkg/adapter/internal/contracttest/mount.go`, `pkg/adapter/reference/adapter_conformance_test.go`, `pkg/adapter/resourceset/adapter_test.go` |
