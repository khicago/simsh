...
| External outcome proof breadcrumbs | SHOULD | Need downstream proof reports to classify command-surface misunderstandings from `ExecutionTrace.ExternalOutcomes` before falling back to compatibility text, while preserving classification source and outcome kind for auditability | `benchmarks/paired_uplift/tasks.go`, `benchmarks/paired_uplift/models.go`, `benchmarks/paired_uplift/reports/*`, `docs/architecture-paired-ab-uplift-proof-harness.md` |
